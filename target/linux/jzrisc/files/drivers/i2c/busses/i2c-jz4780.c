...
/* JZ4780_I2C Control Register (JZ4780_I2C_CTRL) */
/* Stop Hold Enable bit: when tx fifo empty, 0: send stop 1: never send stop */
#define JZ4780_I2C_CTRL_STPHLD		BIT(7)
/* Controls whether SMB has its slave disabled after reset */
...
/* 1: 10bit address 0: 7bit addressing */
#define JZ4780_I2C_CTRL_MATP		BIT(4)
/* 1: 10bit address 0: 7bit address */
...
/* JZ4780_I2C Status Register (JZ4780_I2C_STA) */
/* Slave FSM is not in IDLE state */
#define JZ4780_I2C_STA_SLVACT		BIT(6)
/* Master FSM is not in IDLE state */
...
/* JZ4780_I2C Activity Status */
#define JZ4780_I2C_STA_ACT		BIT(0)

/* JZ4780_I2C Transmit Abort Status Register (JZ4780_I2C_TXABRT) */
...
/* i2c interrupt status (JZ4780_I2C_INTST) */
...
/* i2c interrupt mask status (JZ4780_I2C_INTM) */
...
#define JZ4780_I2C_ENB_I2C		BIT(0) /* Enable the i2c */

/* JZ4780_I2C standard mode high count register(JZ4780_I2CSHCNT) */
#define JZ4780_I2CSHCNT_ADJUST(n)	(((n) - 8) < 6 ? 6 : ((n) - 8))
/* JZ4780_I2C standard mode low count register(JZ4780_I2CSLCNT) */
#define JZ4780_I2CSLCNT_ADJUST(n)	(((n) - 1) < 8 ? 8 : ((n) - 1))
/* JZ4780_I2C fast mode high count register(JZ4780_I2CFHCNT) */
#define JZ4780_I2CFHCNT_ADJUST(n)	(((n) - 8) < 6 ? 6 : ((n) - 8))
/* JZ4780_I2C fast mode low count register(JZ4780_I2CFLCNT) */
			...
			int cnt, int idx)
{
	...
	if (timeout && !i2c->stop_hold) {
		...
		for (timeout = JZ4780_I2C_TIMEOUT*100; timeout > 0; timeout--) {
			i2c_sta = jz4780_i2c_readw(i2c, JZ4780_I2C_STA);
