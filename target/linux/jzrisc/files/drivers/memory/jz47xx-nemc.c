...
/**
 * jz47xx_nemc_num_banks() - count the number of banks referenced by a device
 ...
 */
unsigned int jz47xx_nemc_num_banks(struct device *dev)
{
	const __be32 *prop;
...
EXPORT_SYMBOL(jz47xx_nemc_num_banks);

/**
 * jz47xx_nemc_set_type() - set the type of device connected to a bank
 * @dev: child device of the NEMC.
 * @bank: bank number to configure.
 * @type: type of device connected to the bank.
 */
void jz47xx_nemc_set_type(struct device *dev, unsigned int bank,
			  enum jz47xx_nemc_bank_type type)
{
	struct jz47xx_nemc *nemc = dev_get_drvdata(dev->parent);
	uint32_t nfcsr;
...
EXPORT_SYMBOL(jz47xx_nemc_set_type);

/**
 * jz47xx_nemc_assert() - (de-)assert a NAND device's chip enable pin
 * @dev: child device of the NEMC.
 ...
 * specified bank.
 */
void jz47xx_nemc_assert(struct device *dev, unsigned int bank, bool assert)
{
	struct jz47xx_nemc *nemc = dev_get_drvdata(dev->parent);
	uint32_t nfcsr;
...
EXPORT_SYMBOL(jz47xx_nemc_assert);

static uint32_t jz47xx_nemc_clk_period(struct jz47xx_nemc *nemc)
{
	unsigned long rate;
...
static uint32_t jz47xx_nemc_ns_to_cycles(struct jz47xx_nemc *nemc, uint32_t ns)
{
	return ((ns * 1000) + nemc->clk_period - 1) / nemc->clk_period;
}

static bool jz47xx_nemc_configure_bank(struct jz47xx_nemc *nemc,
				       unsigned int bank,
				       struct device_node *node)
{
	...
	if (of_property_read_u32(node, "ingenic,nemc-tAS", &val) == 0) {
		...
		cycles = jz47xx_nemc_ns_to_cycles(nemc, val);
		if (cycles > 15) {
...
static int jz47xx_nemc_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct jz47xx_nemc *nemc;
	struct resource *res;
	...
	if (!nemc) {
		dev_err(dev, "failed to allocate NEMC data\n");
		return -ENOMEM;
	}
	...
	nemc->clk_period = jz47xx_nemc_clk_period(nemc);
	if (!nemc->clk_period) {
	...
	for_each_child_of_node(nemc->dev->of_node, child) {
		...
		for_each_set_bit(bank, &referenced, JZ47XX_NEMC_NUM_BANKS) {
			if (!jz47xx_nemc_configure_bank(nemc, bank, child)) {
				referenced = 0;
				break;
			}
		}
...
static int jz47xx_nemc_remove(struct platform_device *pdev)
{
	struct jz47xx_nemc *nemc = platform_get_drvdata(pdev);

	clk_disable_unprepare(nemc->clk);
	return 0;
}

static const struct of_device_id jz47xx_nemc_dt_match[] = {
	{ .compatible = "ingenic,jz4780-nemc" },
	{},
};
MODULE_DEVICE_TABLE(of, jz47xx_nemc_dt_match);

static struct platform_driver jz47xx_nemc_driver = {
	.probe		= jz47xx_nemc_probe,
	.remove		= jz47xx_nemc_remove,
	.driver	= {
		.name	= "jz47xx-nemc",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(jz47xx_nemc_dt_match),
	},
};

static int __init jz47xx_nemc_init(void)
{
	return platform_driver_register(&jz47xx_nemc_driver);
}
subsys_initcall(jz47xx_nemc_init);

static void __exit jz47xx_nemc_exit(void)
{
	platform_driver_unregister(&jz47xx_nemc_driver);
}
module_exit(jz47xx_nemc_exit);

MODULE_AUTHOR("Alex Smith <alex.smith@imgtec.com>");
MODULE_DESCRIPTION("Ingenic JZ47xx NEMC driver");
MODULE_LICENSE("GPL");
